...
describe('tooltip.vue', () => {
  ...
  it('basic tooltip', async () => {
    ...
    expect(wrapper.find('.m-tooltip__popper').exists()).toBeFalsy()

    // 弹出层是否出现
    triggerArea.trigger('click')
    await vi.runAllTimers()
    expect(wrapper.find('.m-tooltip__popper').exists()).toBeTruthy()
    expect(wrapper.get('.m-tooltip__popper').text()).toBe('hello tooltip')
  ...
  it('tooltip with hover trigger', async () => {
    ...
    // 测试悬停显示
    wrapper.find('.m-tooltip__trigger').trigger('mouseenter')
    await vi.runAllTimers()
    expect(wrapper.find('.m-tooltip__popper').exists()).toBeTruthy()
    // 测试悬外隐藏
    wrapper.find('.m-tooltip').trigger('mouseleave')
    await vi.runAllTimers()
    expect(wrapper.find('.m-tooltip__popper').exists()).toBeFalsy()
  })
  ...
  it('tooltip with contextmenu trigger', async () => {
    ...
    // 测试右键菜单显示
    wrapper.find('.m-tooltip__trigger').trigger('contextmenu')
    await vi.runAllTimers()
    expect(wrapper.find('.m-tooltip__popper').exists()).toBeTruthy()
    // 测试右键菜单隐藏（可以模拟点击外部区域）
  })
  ...
  it('disabled tooltip', async () => {
    ...
    // 测试禁用状态下点击不会触发显示
    wrapper.find('.m-tooltip__trigger').trigger('click')
    await vi.runAllTimers()
    expect(wrapper.find('.m-tooltip__popper').exists()).toBeFalsy()
  })
